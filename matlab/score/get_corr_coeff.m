function [all_corr_coef, pkg_sort_idx, best_corr_coeff] = get_corr_coeff( r, structure_map, good_idx, structure_tags, BLANK_OUT3, BLANK_OUT5, corr_type, num_show, clip);
...
for n = 1:size( structure_map, 3)   
    ...
    r_subset_vals = r_subset(gp);
    structure_map_subset_vals = structure_map_subset(gp);
    %    R = corrcoef( r_subset(gp), structure_map_subset(gp));
    all_corr_coef(n) = corr(r_subset_vals(:), structure_map_subset_vals(:),'Type',corr_type);
end
...
best_corr_coeff = max(all_corr_coef);
